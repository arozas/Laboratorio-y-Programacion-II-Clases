       ...
       public static void CargarPalabras()
        ...
        public static void MostrarPalabras()  //desarrollar
        {
            for (int i = 0; i < arrayPalabras.Length; i++)
            {
                ...
                Console.WriteLine("{1}.{0}", arrayPalabras[i],i);
            }
        }
        ...
        {
            MostrarPalabras();
            Console.WriteLine("\nIngrese el numero de la palabra a eliminar:");
            string palabraEliminar = Console.ReadLine();
            if (!string.IsNullOrWhiteSpace(palabraEliminar) && int.TryParse(palabraEliminar, out int indice) && indice != -1)
            {
                arrayPalabras[indice] = null;
                Console.WriteLine("Palabra eliminada.");
                Funciones.PresioneUnaTeclaParaContinuar();
            }
            else
            {
                Console.WriteLine("Error. Debe ingresar un indice valido.");
            }
